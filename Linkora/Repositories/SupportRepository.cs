namespace Linkora.Repositories
{
    public class SupportRepository : SqlRepositoryBase, ISupportRepository
    {
        ...
        public async Task<int> CreateRequestAsync(string name, string email, string? phone, string message, string? userId) => (await QueryAsync<int>(
                @"INSERT INTO SupportRequests (Name, Email, Phone, Message, CreatedAt, Status, UserId) OUTPUT INSERTED.Id VALUES (@Name, @Email, @Phone, @Message, @CreatedAt, @Status, @UserId)",
                r => r.GetInt32(0),
                p =>
                {
