...
namespace Linkora.Repositories
{
    public class PromotionRepository : SqlRepositoryBase, IPromotionRepository
    {
        ...
        public async Task<Promotion?> GetActiveAsync(int userId) => await QuerySingleAsync(
            @"SELECT TOP 1 Id, UserId, Tier, TermType, StartedAt, ExpiresAt, PricePaid, Status
              FROM Promotion
        ...
        public async Task<int> CreateAsync(int userId, PromotionTier tier, PromotionTermType termType, DateTime startedAt, DateTime expiresAt, decimal pricePaid) => (await QueryAsync<int>(
            @"INSERT INTO Promotion (UserId, Tier, TermType, StartedAt, ExpiresAt, PricePaid, Status)
              OUTPUT INSERTED.Id
              VALUES (@UserId, @Tier, @TermType, @StartedAt, @ExpiresAt, @PricePaid, @Active)",
            r => r.GetInt32(0),
            p =>
            {
                p.AddWithValue("@UserId", userId);
        ...
        public async Task SupersedeAsync(int promotionId) => await ExecuteAsync(
            "UPDATE Promotion SET Status = @Superseded WHERE Id = @Id",
            p =>
            {
        ...
        public async Task<int> ExpireDueAsync() => await ExecuteAsync(
            "UPDATE Promotion SET Status = @Expired WHERE Status = @Active AND ExpiresAt <= SYSUTCDATETIME()",
            p =>
            {
