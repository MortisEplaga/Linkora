    ...
    public class CompareRepository : SqlRepositoryBase, ICompareRepository
    {
        ...
        public async Task<CompareData> GetCompareDataAsync(int userId, string lang)
        {
            ...
            var paramRows = await QueryAsync(
                ...
                p => { foreach (var prm in productParams) p.Add(prm); });

            var selectParamIds = paramRows
                .Where(x => x.ParamType == 2 || x.ParamType == 4 || x.ParamType == 8)
                .Select(x => x.ParamId)
                .Distinct()
                .ToList();

            var selectOptionsDict = selectParamIds.Count > 0
            ...
            foreach (var row in paramRows)
            {
                ...
                if (row.ParamType == 4)
                {
                    ...
                    foreach (var idStr in rawValue.Split(',', StringSplitOptions.RemoveEmptyEntries))
                ...
                if (!result.ParamMatrix.ContainsKey(row.ParamId)) result.ParamMatrix[row.ParamId] = [];
